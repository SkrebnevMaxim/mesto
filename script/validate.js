...
function enableValidation(validationConfig) { // функция включения валидации
...
function setFormListener(form, config) {  // функция обработки формы
...
function setSubmitButtonState(form, config) { //функция изменения кнопки, если валидация не пройдена, кнопка не активна и меняет цвет
  const button = form.querySelector(config.submitButtonSelector);

  button.disabled = !form.checkValidity();
  button.classList.toggle(config.inactiveButtonClass, !form.checkValidity())
}
function handlerSubmit(evt) { // функция сохранения без перезагрузки страницы
  evt.preventDefault()
}

function handleFieldValidation(input, form, config) { //функция обработки формы на валидность
  if (!input.validity.valid) { // пишем условия добавления или скрытия ошибок валидации
  showError(input, form, config)
  } else {
  hideError(input, form, config)
  }
}
function showError(input, form, config) { //функция показа ошибки валидации
...
function hideError(input, form, config) { // функция скрытия ошибки при выполнении условий валидации
